...
/// Payload for the before_sync hook (called after auth, before y-sync starts).
...
pub trait Hook: Send + Sync {
    ...
    ///
    /// Use this for custom handshakes like Files-Ready protocol:
    /// 1. Return `SendMessages` with file manifest
    /// 2. Client receives manifest, downloads files
    /// 3. Client sends FilesReady (handled by `on_control_message`)
    /// 4. Hook sends CrdtState via `sender`
