...
use kameo::{
    actor::{Actor, ActorId, ActorRef, Spawn, WeakActorRef},
    ...
    fn on_link_died(
        ...
        if let Some(doc_id) = doc_id {
            ...
            for hook in self.hooks.iter() {
                hook.after_unload_document(&doc_id);
            }
        }
...
use crate::actor::messages::{
    ApplyServerUpdate, BroadcastText, CreateClient, PersistDocument, PersistNow, RequestDoc,
};
...
impl Message<RequestDoc> for Root {
    ...
    async fn handle(
        &mut self,
        RequestDoc(doc_id): RequestDoc,
        ctx: &mut Context<Self, Self::Reply>,
    ) -> Self::Reply {
        if let Some(doc) = self.active_docs.get(&doc_id) {
            return doc.clone();
        }
        let doc = DocActor::spawn_link(
            ctx.actor_ref(),
            DocActorArgs {
...
impl Message<PersistDocument> for Root {
    ...
    async fn handle(
        &mut self,
        PersistDocument(doc_id): PersistDocument,
...
impl Message<GetDocPeerCount> for Root {
    ...
    async fn handle(
        &mut self,
        GetDocPeerCount(doc_id): GetDocPeerCount,
        _: &mut Context<Self, Self::Reply>,
    ) -> usize {
        if let Some(doc) = self.active_docs.get(&doc_id) {
